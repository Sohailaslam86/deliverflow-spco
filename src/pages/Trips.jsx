      ...
      {showForm && (
        <Card>
          ...
          <div style={{ display: \"grid\", gridTemplateColumns: \"1fr 1fr\", gap: \"12px\" }}>
            ...
            <Select label={t.destCity} value={form.toCity} onChange={v => setForm({ ...form, toCity: v })} options={CITIES.filter(c => c !== dc)} required />
            ...
            <Select label={t.storage} value={form.storage} onChange={v => setForm({ ...form, storage: v })} options={STORAGE_CONDITIONS.map(s => s.name + \" (\" + s.range + \")\")} />
        ...
        {[...myTrips].reverse().map(trip => (
          <div key={trip.id} style={{ border: \"1px solid #e2e8f0\", borderRadius: 8, padding: 14, marginBottom: 8 }}>
            ...
            <div style={{ display: \"grid\", gridTemplateColumns: \"repeat(auto-fit,minmax(160px,1fr))\", gap: 6, fontSize: 13 }}>
              ...
              {trip.invoiceIds?.length > 0 && <div>📋 <b>{t.invoices}</b> {trip.invoiceIds.length}</div>}
            </div>
