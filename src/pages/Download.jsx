    ...
    <table>
      ...
      <tbody>
        ${data.map(inv=>`<tr>
          ...
          <td>${inv.podImage&&inv.podImage!=="demo_pod"?`<img src="${inv.podImage}" class="pod-img"/>`:"-"}</td>
        </tr>`).join("")}
      </tbody>
    </table>
  ...
  if (viewInv) {
    return (
      <div style={{ direction:rtl?"rtl":"ltr" }}>
        <button onClick={()=>setViewInv(null)} style={{ background:"#1A3A5C",color:"white",border:"none",padding:"12px 22px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:14,marginBottom:16 }}>
          ← {t.back}
        </button>
        <Card>
          <CardTitle>📸 POD — {viewInv.id}</CardTitle>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:10,marginBottom:16,fontSize:14 }}>
          ...
          {viewInv.podImage&&viewInv.podImage!=="demo_pod"?(
            ...
            <div style={{ background:"#f8fafc",borderRadius:10,padding:40,textAlign:"center",color:"#94a3b8",fontSize:15 }}>
              {t.noPOD}
            </div>
          )}
        </Card>
    ...
    <div style={{ direction:rtl?"rtl":"ltr" }}>
      ...
      <Card>
        ...
        <div style={{ display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:12 }}>
          ...
          {/* DC Dropdown — always show all options for admin/planning */}
          {(isAdmin||isPlanning||!userDC)?(
            <select value={filter} onChange={e=>setFilter(e.target.value)}
              style={{ border:"1.5px solid #e2e8f0",borderRadius:8,padding:"11px 14px",fontSize:15,background:"white",outline:"none" }}>
              <option value="all">{t.allDCs}</option>
              {DCS.map(dc=><option key={dc} value={dc}>{dc} DC</option>)}
            </select>
          ):(
        ...
        <div style={{ display:"flex",gap:8,flexWrap:"wrap",marginBottom:14,alignItems:"center" }}>
          <span style={{ fontSize:14,color:"#64748b" }}>{filtered.length} {t.delivered}</span>
          <div style={{ flex:1 }} />
          <Btn small onClick={()=>setSelected(filtered.map(i=>i.id||i.firestoreId))} color="#6366f1">{t.selectAll}</Btn>
              ...
              <thead>
                <tr style={{ background:"#f8fafc" }}>
                  ...
                  {[t.invoice,t.date,t.customer,t.dc,t.driver,t.vehicle,"POD"].map(h=>(
                    <th key={h} style={{ padding:"12px 14px",textAlign:"left",fontWeight:700,color:"#374151",borderBottom:"2px solid #e2e8f0",whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((inv,i)=>{
                  ...
                  return (
                    <tr key={invId} style={{ background:selected.includes(invId)?"#eef2ff":i%2===0?"white":"#f8fafc",cursor:"pointer" }}
                      onClick={()=>toggleSelect(invId)}>
                      <td style={{ padding:"12px 8px",textAlign:"center" }}>
                        <span style={{ fontSize:18,color:"#6366f1" }}>{selected.includes(invId)?"☑":"☐"}</span>
                      </td>
                      <td style={{ padding:"12px 14px",fontWeight:700,color:"#1A3A5C" }}>{inv.id}</td>
                      <td style={{ padding:"12px 14px",color:"#64748b" }}>{inv.date}</td>
                      <td style={{ padding:"12px 14px" }}>{inv.customer}</td>
                      <td style={{ padding:"12px 14px" }}>{inv.dc}</td>
                      <td style={{ padding:"12px 14px",color:"#64748b" }}>{inv.driverName||inv.driver||"-"}</td>
                      <td style={{ padding:"12px 14px",color:"#64748b" }}>{inv.vehicle||"-"}</td>
                      <td style={{ padding:"12px 14px" }} onClick={e=>{e.stopPropagation();setViewInv(inv);}}>
                        <span style={{ color:"#6366f1",fontWeight:600,cursor:"pointer",fontSize:13 }}>
